async function user(fastify, {dbInterface}) {
    fastify.post('/', async function (request, reply) {
        ...
        const {error, dbResp} = await dbInterface.user.createInDB({name, email, password})
    ...
    fastify.get('/', async function (request, reply) {
        ...
        if (!uid && !email) { 
            ...
            const {error, dbResp} = await dbInterface.user.readInDB({uid: uid})
            if (error) return reply.code(400).send({error: error})
            return reply.code(200).send(dbResp)
        } else if (!uid && email) {
            const {error, dbResp} = await dbInterface.user.readInDB({email: email})
            if (error) return reply.code(400).send({error: error})
            return reply.code(200).send(dbResp)
        } else { // both uid and tid
            const uidDbResp = await dbInterface.user.readInDB({uid: uid})
            if (uidDbResp.error) return reply.code(400).send({error: uidDbResp.error})
            const emailDbResp = await dbInterface.user.readInDB({email: email})
    ...
    fastify.put('/', async function (request, reply) {
        ...
        const {error, dbResp} = await dbInterface.user.updateInDB({uid: uid, name: name, email: email, password: password}) 
        if (error) return reply.code(400).send({error: error})
        return reply.code(200).send(dbResp)
    })
    fastify.delete('/', async function (request, reply) {
        ...
        if (!uid) return reply.code(400).send({ error: 'Missing uid' });
        const {error, dbResp} = await dbInterface.user.deleteInDB({uid: uid}) 
        if (error) return reply.code(400).send({error: error})
        return reply.code(200).send(dbResp)
    })
}
